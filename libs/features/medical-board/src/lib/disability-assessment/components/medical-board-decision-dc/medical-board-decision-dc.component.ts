...
import { AssessmentResponseDateDto, Contributor, OriginLocation, Person, RescheduleSessionData } from '../../../shared';
import { BsModalRef, BsModalService } from 'ngx-bootstrap/modal';
import { showModal } from '@gosi-ui/features/benefits/lib/shared';
import moment from 'moment';
@Component({
  selector: 'mb-medical-board-decision-dc',
  templateUrl: './medical-board-decision-dc.component.html',
  styleUrls: ['./medical-board-decision-dc.component.scss']
})
export class MedicalBoardDecisionDcComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @Input() modifiedByGosiDoc = false;
  @Input() ambReturn = false;
  @Input() isHoReturn = false;
  ...
  selectResult(result) {
    ...
    if (
      (this.assessmentType === 'Non-Occupational Disability' ||
        ...
        this.assessmentType === 'Heir Disability Assessment' ||
        this.assessmentType === 'Reassessment Non-Occupational Disability' ||
        this.assessmentType === 'Non - Occupational Disability Reassessment' ||
        this.assessmentType === 'Reassessment Heir Disability' ||
        this.assessmentType === 'Reassessment Dependent Disability' ||
      ...
      if (
        (this.assessmentDetails?.disabilityDate && !this.isGosiDoctor && !this.isReturn && !this.ambReturn) ||
        ((this.isGosiDoctor || this.ambReturn) &&
          !this.gosiDoctorAssessDetails?.disabilityDate?.gregorian &&
          this.assessmentDetails?.disabilityDate)
      ) {
        this.boardDecisonForm
          .get('disabilityDate')
          ?.get('gregorian')
          .setValue(this.formatDate((this.assessmentDetails?.disabilityDate?.gregorian)));
        this.selectDisabilityDate();
      }
      if (
        (this.isGosiDoctor || this.isReturn || this.ambReturn)) {
        if (this.gosiDoctorAssessDetails?.modifiedDetails?.disabilityDate?.gregorian) {
          this.boardDecisonForm
            .get('disabilityDate')
            ?.get('gregorian')
            .setValue(this.formatDate((this.gosiDoctorAssessDetails?.modifiedDetails?.disabilityDate?.gregorian)));
        }
        else if (this.gosiDoctorAssessDetails?.disabilityDate?.gregorian) {
          this.boardDecisonForm
            .get('disabilityDate')
            ?.get('gregorian')
            .setValue(this.formatDate((this.gosiDoctorAssessDetails?.disabilityDate?.gregorian)));
        }
      }
      if (this.modifiedByGosiDoc && this.gosiDoctorAssessDetails?.mainReason) {
        this.boardDecisonForm?.get('mainReason')?.patchValue(this.gosiDoctorAssessDetails?.mainReason);
      }
      // if (this.modifiedByGosiDoc &&this.gosiDoctorAssessDetails?.modifiedDetails?.secondaryReason) {
      //   this.boardDecisonForm
      //     ?.get('secondaryReason')
      //     ?.patchValue(this.gosiDoctorAssessDetails?.modifiedDetails?.secondaryReason);
      // }
      if (this.modifiedByGosiDoc && this.gosiDoctorAssessDetails?.secondaryReason) {
        this.boardDecisonForm?.get('secondaryReason')?.patchValue(this.gosiDoctorAssessDetails?.secondaryReason);
      }
      if (this.modifiedByGosiDoc && this.gosiDoctorAssessDetails?.nextAssessmentDate?.gregorian) {
        this.boardDecisonForm.addControl(
          'nextAssessmentDate',
          this.fb.group({ gregorian: ['', Validators.required], hijiri: [''] })
        );
        this.boardDecisonForm
          ...
          ?.get('gregorian')
          .setValue(this.formatDate(this.gosiDoctorAssessDetails?.nextAssessmentDate?.gregorian));
      }
      if (this.modifiedByGosiDoc && this.gosiDoctorAssessDetails?.primaryGosiDr?.name) {
  ...
  selectRescheduleReason(rescheduleReason) {
    ...
    } else if (rescheduleReason === 'Required clarification from contributor' || rescheduleReason === 'Required clarification from heir') {
  ...
  setGosiDoctorAssessment() {
    if (this.gosiDoctorAssessDetails) {
      ...
      if (this.gosiDoctorAssessDetails?.isHelperRequired?.english === 'Yes') {
        if (this.gosiDoctorAssessDetails?.modifiedDetails?.reasonForHelper) {
        ...
        if (this.gosiDoctorAssessDetails?.modifiedDetails?.isHelperRequired) {
          this.helperRequired = this.gosiDoctorAssessDetails?.modifiedDetails?.isHelperRequired?.english;
        } else {
        ...
        if (this.gosiDoctorAssessDetails?.modifiedDetails?.helperStartDate?.gregorian) {
          this.boardDecisonForm
            ?.get('helperStartDate')
            ?.get('gregorian')
            .setValue(this.formatDate(this.gosiDoctorAssessDetails?.modifiedDetails?.helperStartDate?.gregorian));
        }
        else if (this.gosiDoctorAssessDetails?.helperStartDate?.gregorian) {
          this.boardDecisonForm
            ?.get('helperStartDate')
            ?.get('gregorian')
            .setValue(this.formatDate(this.gosiDoctorAssessDetails?.helperStartDate?.gregorian));
        }
      ...
      if (this.gosiDoctorAssessDetails?.nextAssessmentDate) {
        ...
        if (this.gosiDoctorAssessDetails?.modifiedDetails?.nextAssessmentDate) {
          this.boardDecisonForm
            ?.get('nextAssessmentDate')
            ?.get('gregorian')
            .setValue(this.formatDate(this.gosiDoctorAssessDetails?.modifiedDetails?.nextAssessmentDate?.gregorian));
        } else {
          this.boardDecisonForm
            ?.get('nextAssessmentDate')
            ?.get('gregorian')
      ...
      this.boardDecisonForm
        ...
        ?.get('gregorian')
        .setValue(this.formatDate((this.gosiDoctorAssessDetails?.disabilityEndDate?.gregorian)));
    }
  }
  ...
  selectDisabilityDate() {
    this.startDate = moment(
      convertToYYYYMMDD(this.boardDecisonForm?.get('disabilityDate')?.get('gregorian')?.value)
    ).toDate();
    this.endDate = this.today;
  }
  getDisabledReschedule() {
    if (this.modifiedByGosiDoc) {
      return false;
    } else if (this.conveyanceRequired || this.isReturn || this.isHoReturn) {
      return true;
    } else {
      return false;
    }
