...
import { ContractAuthConstant, EngagementBasicDetails, EngagementDetails, UpdatedWageDetails } from '@gosi-ui/features/contributor/lib/shared';
...
export class VerifyEngagementDetailsDcComponent implements OnInit, OnChanges {
  ...
  wageDetails: UpdatedWageDetails = new UpdatedWageDetails();
  ...
  /** Input  variables. */
  @Input() modifyEngagement:EngagementBasicDetails = new EngagementBasicDetails();
  ...
  @Input() isModifyWageAndOccupation: boolean;
  ...
  ngOnInit(): void {
    //console.log(this.modifyEngagement,'modify eng');
    ...
    this.engStartDate = convertToStringDDMMYYYY(
      this.engagementDetails.joiningDate.gregorian.toString()
    );
    if(this.engagementDetails.status === "LIVE" || this.engagementDetails.status === "TERMINATION_IN_PROGRESS" || this.engagementDetails.status === "CANCEL_IN_PROGRESS"){
      this.engagementStatus = "Active"
    }
    else{
      this.engagementStatus = "Inactive"
    }
    this.engagementLeavingDate = convertToStringDDMMYYYY(moment(this.engagementDetails?.leavingDate?.gregorian).toString());
  }
  ...
  if (changes.modifyEngagement && changes.modifyEngagement.currentValue) {
    //console.log(this.modifyEngagement.violationPeriodRequests);
    this.setWageDetails();
...
setWageDetails(){
    ...
    this.engagementDetails.engagementPeriod.forEach(oldPeriod=>{
      const oldPeriodEndDate=oldPeriod.endDate ? new Date(oldPeriod.endDate.gregorian) : currentDate;
      const updatedPeriodList=[];
      this.modifyEngagement.violationPeriodRequests.forEach(updatedPeriod=>{
        const updatedPeriodEndDate=updatedPeriod.endDate ? new Date(updatedPeriod.endDate.gregorian) : currentDate;
        if((new Date(updatedPeriod.startDate.gregorian) >= new Date(oldPeriod.startDate.gregorian))
          && (updatedPeriodEndDate <= oldPeriodEndDate) 
          && (updatedPeriod.wageDetailsUpdated)
          ){
            updatedPeriodList.push(updatedPeriod);
        
          }
         
      });
      this.wageDetails.wagePeriods.push({current :oldPeriod ,updated :updatedPeriodList});
    })
    //console.log(this.wageDetails.wagePeriods[0],'periods');
  }
  /**
