...
import {
  ...
  import { LanguageToken, Contributor, CommonIdentity, getPersonName, checkNull, getIdentityByType, Name } from '@gosi-ui/core';
  ...
  @Component({
    selector: 'oh-auditor-allowance-detail-oh-dc',
    templateUrl: './auditor-allowance-detail-oh-dc.component.html',
    styleUrls: ['./auditor-allowance-detail-oh-dc.component.scss']
  })
  export class AuditorAllowanceDetailOhDcComponent implements OnInit, OnChanges, AfterViewChecked {
    lang: string;
    modalRef: BsModalRef;
    ...
    ngOnChanges(changes: SimpleChanges) {
      if (changes && changes.allowanceSummaryDetails) {
        ...
        this.contributor.english.name = this.allowanceSummaryDetails.contributorName;
        this.refId = this.allowanceSummaryDetails.ninIqama;
      }
    }
    ...
    ngAfterViewChecked() {
      ...
      if (this.allowanceSummaryDetails) {
        this.contributor.english.name = this.allowanceSummaryDetails.contributorName;
        this.refId = this.allowanceSummaryDetails.ninIqama;
        if(this.lang === 'en') {
          this.providerNameEnglish = this.getProviderName(this.allowanceSummaryDetails.providerName, 'en');
