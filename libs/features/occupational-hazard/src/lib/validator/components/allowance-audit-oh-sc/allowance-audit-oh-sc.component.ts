...
import { AllowanceOhList } from '../../../shared/models/allowance-oh-list';
import { AllowanceAuditSummaryOh } from '../../../shared/models/allowance-audit-summary-oh';
import { ClosedAllowanceDetails } from '../../../shared/models/closed-allowance-details';
@Component({
  selector: 'oh-allowance-audit-oh-sc',
  templateUrl: './allowance-audit-oh-sc.component.html',
  styleUrls: ['./allowance-audit-oh-sc.component.scss']
})
export class AllowanceAuditOhScComponent extends AllowanceBaseScComponent implements OnInit {
  ...
  auditDetails: AllowanceOhList = new AllowanceOhList();
  auditDetailsCompletion: AllowanceOhList = new AllowanceOhList();
  auditHeaderDetails: AllowanceOhList = new AllowanceOhList();
  ...
  selectedIds = [];
  auditSummaryOh: AllowanceAuditSummaryOh = new AllowanceAuditSummaryOh();
  auditStatus: boolean;
  closedAllowances: number[];
  requestedAllowances: number[];
  ...
  fetchAllowance(allowance: AllowanceSummary) {
    ...
    // }, 1000);
    // setTimeout(() => {
    //   this.claimsService.fetchAuditAllowanceDetails(allowance.caseId);
    // }, 1000);
  }
  fetchAllowanceSummary(caseId) {
    this.claimsService.fetchAllowanceSummaryOh(caseId, this.referenceNo).subscribe(
      response => {
        this.auditSummaryOh = response;
      },
      err => {
        this.showError(err);
      }
  ...
  navigateTo(details: AllowanceSummary) {
    ...
    this.regNo = details.registrationNo;
    this.sin = details.socialInsuranceNo;
    this.ohService.setRegistrationNo(this.regNo);
    this.ohService.setSocialInsuranceNo(this.sin);
    this.ohService.setRoute(Route.CONTROLLER_ALLOWANCE_AUDIT);
  ...
  showModal(templateRef: TemplateRef<HTMLElement>, size, selectedAllowance?) {
    ...
    if (selectedAllowance) {
      selectedAllowance.forEach(service => {
        this.selectedIds.push(service.ohClaimId);
      });
    }
  ...
  approveWorkflow() {
    ...
    this.workflowService.updateAllowanceTaskWorkflow(approveworkflowData).subscribe(
      () => {
  ...
  // ALLOWANCE AUDIT NEW CHANGES
  getAuditDetails() {
    this.claimsService.getAuditOhDetails(this.referenceNo).subscribe(
      response => {
        this.auditDetails = response;
        ...
        this.auditDetails.batchMonth = response.monthOfSubmission;
        this.auditDetails.auditCases = response.allowanceAuditCases;
        this.auditDetails.auditCases.forEach((item, index) => {
          item.caseId = response.auditCases[index].injuryId;
        });
      },
      err => {
        this.showError(err);
  ...
  rejectEvent(closedDetails) {
    ...
    //     }
    //   );
    this.closedAllowances = [];
      this.claimsService.closeAllowance(closedDetails.comments, closedDetails.ohClaimIds, this.injuryId, this.referenceNo).subscribe(res => {
        if(res){
          this.closedAllowances = closedDetails.ohClaimIds;
          this.alertService.showSuccessByKey('OCCUPATIONAL-HAZARD.CLOSED-ALLOWANCES');
          // this.fetchAllowanceSummary(this.injuryId);
          setTimeout(() => {
            this.alertService.clearAlerts();
          }, 5000);
        }
  ...
  requestAllowanceTpa(){
    const workflowData = new ClosedAllowanceDetails();
    workflowData.comments = this.reportAllowanceModal.get('comments').value;
    workflowData.ohClaimIds = this.selectedIds;
    workflowData.documents = this.tpaRequestedDocs;
    // this.selectedAllowance.forEach(element => {
    //   workflowData.ohClaimIds.push(element.ohClaimId);
    // });
    // this.requestedAllowances = [];
    if (this.reportAllowanceModal && this.reportAllowanceModal?.valid) {
      this.claimsService.requestTpaAllowance(workflowData, this.injuryId, this.referenceNo).subscribe(res => {
          if(res){
            this.resetModalAllowance();
            this.requestedAllowances = this.selectedIds;
            this.alertService.showSuccessByKey('OCCUPATIONAL-HAZARD.REQUEST-ALLOWANCES');
            setTimeout(() => {
              this.alertService.clearAlerts();
            }, 5000);
            // this.fetchAllowanceSummary(this.injuryId);
  ...
  markAsCompleted() {
    ...
    this.claimsService.getAuditOhDetails(this.referenceNo).subscribe(
      response => {
        this.auditDetailsCompletion = response;
        if(this.auditDetailsCompletion.auditStatus === 'In Progress'){
          this.alertService.showErrorByKey('OCCUPATIONAL-HAZARD.NOT-ALLOWANCE-COMPLETED');
          setTimeout(() => {
            this.alertService.clearAlerts();
          }, 7000);
          this.auditStatus = true
