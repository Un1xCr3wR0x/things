...
import { Component, EventEmitter, Input, OnChanges, OnInit, Output, SimpleChanges, TemplateRef, ViewChild, Inject } from '@angular/core';
import { FormBuilder, FormGroup, Validators, FormControl } from '@angular/forms';
import { BaseComponent, DocumentItem, iBanValidator, lengthValidator, Lov, LovList, AddressDetails, AddressTypeEnum, Person, LookupService, ApplicationTypeToken, LanguageToken, DocumentService, AlertService, bindToObject, NIN, Iqama, NationalId, Passport } from '@gosi-ui/core';
...
export class AddBankDetailsFormDcComponent extends ChangePersonScBaseComponent implements OnInit{
  ...
  warningMessage: string = "The bank account number (IBAN) will be verified. Please ensure that the bank account is active and associated with the account holder to avoid rejection.";
...
/**
  ...
  constructor(  readonly changePersonService: ChangePersonService, readonly activatedRoute: ActivatedRoute,
    readonly lookService: LookupService,@Inject(ApplicationTypeToken) readonly appToken: string,
    @Inject(LanguageToken) readonly language: BehaviorSubject<string>,readonly alertService: AlertService,
    readonly documentService: DocumentService, private fb: FormBuilder,private manageService :ManagePersonService,readonly modalService: BsModalService,readonly route: ActivatedRoute,readonly router: Router,) {

    super(changePersonService, lookService, appToken, alertService, documentService, modalService, route);
   }
  ...
  submit(){
     ...
     if (this.bankDetailsForm.get('ibanBankAccountNo').value && this.bankDetailsForm.get('bankName').value.english) {
       if (this.showNonSaudi == true){
        this.manageService.saveBankDetails( this.personId,this.bankDetails).subscribe(
          (data:any)=>{
  ...
  createBankForm(): FormGroup {
    return this.fb.group({
      ...
      swiftCode: [null, { validators: Validators.pattern('[a-zA-Z0-9]+'),
         updateOn: 'blur' }],
      bankCode: [null]
    });
  }
