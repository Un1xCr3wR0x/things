...
export class WageBreakupDcComponent extends WageDetailFormBase implements OnInit, OnChanges {
  ...
  editWage(isSelect: boolean) {
    ...
    this.alreadySavedWage =
      typeof period?.wage?.totalWage === 'string' ? parseFloat(period?.wage?.totalWage) : period?.wage?.totalWage;
    this.editMode = true;
    ...
    } else if (this.checkPeriodNeedSplit && this.isAppPublic) {
      if (isSelect) this.periodSelect.emit(this.periodIndex);
      this.wageForm.reset();
      if (period.occupation) {
        this.wageForm.get('occupation').patchValue(period.occupation);
  ...
  saveWageChanges() {
    if (this.wageForm.valid) {
      ...
      this.updatedTotalWage = parseFloat(this.wageForm?.get('wage')?.get('totalWage').value);
      this.calculateWageDifference();
      }
    } else {
  ...
  calculateWageDifference() {
    ...
    if (this.wageDiff >= 50 || this.wageDiff <= -50){
      this.showModal(this.wageDifferenceModal, false);
    } else {
  ...
  resetWage(){
    const period: EngagementPeriod = JSON.parse(JSON.stringify(this.selectedPeriod));
    this.wageForm?.get('wage').reset(period?.wage);
    this.wageForm?.get('wage')?.updateValueAndValidity();
    this.hideModal();
  }
    ...
    if (this.changesInPeriod.length > 0) {
      ...
      this.changesInPeriod.forEach(item => {
        if (item.periodId === this.selectedPeriod.id && this.checkPeriodEligibility(item.startDate, item.endDate)) {
          ...
          this.disableJobValues = !item.isRankGradeClassChanged;
  ...
  patchValues(wageDetails: EngagementPeriod) {
    if (
      wageDetails &&
      this.wageForm &&
      (this.jobClassLov?.length > 0 || this.jobRankLov?.length > 0 || this.jobGradeLov?.length > 0)
    ) {
