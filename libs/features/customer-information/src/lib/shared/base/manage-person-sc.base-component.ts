...
import { ChangePersonService, ManagePersonRoutingService, ManagePersonService } from '../services';
interface BPM {
...
export abstract class ManagePersonScBaseComponent extends BaseComponent implements OnDestroy {
  ...
  hasPassportWorkFlow = false;
  isPassportReturned = false;
  ...
  initialiseTheView(isBorder?: boolean,type?:string) {
    ...
    if (this.manageService.contributor$) {
      return this.manageService.contributor$.pipe(
        ...
        switchMap(contributor => {
          return this.initialiseForContributor(contributor, isBorder,type).pipe(
            switchMap(() => {
  ...
  initialiseForContributor(contributor: Contributor, isBorder: boolean,type:string) {
    if (contributor?.socialInsuranceNo) {
      ...
      this.isPassportActive=contributor.hasLiveEngagementInEstablishment;
      if (this.isIndividual) {
        return this.getWorkFlowStatus(this.socialInsuranceNo, isBorder).pipe(
          tap(() => {
            ...
            this.getDocuments(isBorder,type);
          })
        );
      }
      ...
      this.manageService
        ...
        .pipe(
          ...
          tap(() => {
            ...
            this.initialiseTheView(isBorder,'isAddPassport').subscribe();
          })
        )
  ...
  getWorkFlowStatus(sin: number, isBorder?: boolean) {
    ...
    this.hasPassportWorkFlow=false
          ...
          if (res) {
            res.map(item => {
              if (item.message && item.message !== null) {
                ...
                else if(item.type ===IdentityTypeEnum.ADD_PASSPORT){
                  this.hasPassportWorkFlow = true;
                  this.passportReferenceNo = item.referenceNo;
                  if (item.status === ManagePersonConstants.STATUS_RETURNED) {
                    this.isPassportReturned = true;
                  } else {
                    this.isPassportReturned = false;
                  }
                  this.passportComments = item.transactionData;
                 // console.log(this.passportForm,item.expiryDate)

                   if (this.passportForm.get('passportAdd') && (this.editTransaction || this.manageService.isEdit)) {

                    this.passportForm.get('passportAdd').get('passportNumber').setValue(item?.newValue);
                    const issueDate:any=item?.issueDate
                    const ExpiryDate:any=item?.expiryDate
                    this.passportForm.get('passportAdd').get('passportIssueDate').get('gregorian').setValue(moment(issueDate.gregorian).tz('Asia/Riyadh').toDate());
                    this.passportForm.get('passportAdd').get('passportExpiryDate').get('gregorian').setValue(moment(ExpiryDate.gregorian).tz('Asia/Riyadh').toDate());
                  //  console.log(this.passportForm.get('passportAdd').get('passportExpiryDate').get('gregorian'))
                              }
                  if (this.hasPassportWorkFlow && isBorder === false) {
                    this.initialiseForEditTransaction(item);
                  }
                }
  ...
  initialiseTransaction(isBorder: boolean,type:string) {
    ...
    if (!this.editTransaction) {
      this.uuid = this.uuidService.getUuid();
      return this.initialiseTheView(isBorder,type);
    }
    return of(null);
  }
  ...
  getDocuments(isBorder: boolean,type:string) {
    const identifiers = this.getDocumentIdentifiers(isBorder,type);
    if(type=='isAddPassport' && !this.editTransaction ){
      this.documents$ = this.documentService
      .getRequiredDocuments(identifiers.key, identifiers.documentType)
      .pipe(
        tap(res => {
          if (isBorder) this.borderDocuments = [...res];
          else if (type=='isAddPassport')  this.passportDocuments = [...res];
          else {
      ...
      .pipe(
        ...
        this.documents$ = this.documentService
      .getDocuments(identifiers.key, identifiers.documentType, identifiers.identifier, this.passportReferenceNo)
      .pipe(
        tap(res => {
          if (isBorder) this.borderDocuments = [...res];
          else if (type=='isAddPassport')  this.passportDocuments = [...res];
  ...
  refreshDocument(isBorder, document: DocumentItem) {
    const identifiers = this.getDocumentIdentifiers(isBorder,'isAddPassport');
    this.documentService.refreshDocument(document, identifiers.identifier).subscribe(res => (document = res));
  }
  getDocumentIdentifiers(isBorder: boolean,type) {
    let key;
    let documentType;
    let identifier = null;
    ...
    } else if(type=='isAddPassport'){
      key = ManagePersonConstants.PASSPORT_DOCUMENT_KEY;
      documentType = ManagePersonConstants.PASSPORT_DOCUMENT_TYPE;
    }
    else{
  ...
  navigateToInitialPage(router: Router, isFromProfile: boolean) {
    if (isFromProfile) {
      let identifier = this.changePersonService.getURLId();
      router.navigate([RouterConstants.ROUTE_INDIVIDUAL_PROFILE_INFO(identifier)]);
