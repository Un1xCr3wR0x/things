...
import { BilingualText, RoleIdEnum, RouterConstants } from '@gosi-ui/core';
import { MbRouteConstants } from '../../constants';
import {
  AssessedDoctorDetails,
  AssessmentDetail,
  DisabilityDetails,
  AssessmentDetails,
  Contributor
...
@Component({
  selector: 'mb-assessment-details-view-dc',
  templateUrl: './assessment-details-view-dc.component.html',
  styleUrls: ['./assessment-details-view-dc.component.scss']
})
export class AssessmentDetailsViewDcComponent implements OnInit, OnChanges {
  limitvalue: number;
  limit = 100;
  readMore = false;
  ...
  //     `/${this.socialInsuranceNo}/${this.personId}/${eachperson?.injuryId || eachperson.benefitReqId}`;
