...
import { AllowanceAuditSummaryOh } from '../../models/allowance-audit-summary-oh';
  ...
  @Component({
    selector: 'oh-case-list-audit-dc',
    templateUrl: './case-list-audit-dc.component.html',
    styleUrls: ['./case-list-audit-dc.component.scss']
  })
  export class CaseListAuditDcComponent implements OnInit, OnChanges, AfterViewChecked {
    @Input() auditDetails: AllowanceOhList;
    @Input() allowanceDetails: AllowanceAuditSummaryOh;
    ...
    @Output() rejectEvent: EventEmitter<Object> = new EventEmitter();
    ...
    ngOnInit(): void {
      ...
      this.pageLimit = 10;
      console.log('audittt ', this.auditDetails.auditCases);
    }
    /**
     *
    ...
    rejectAllowance(event) {
      this.selectedAllowancelist.forEach(selectAllownace => {
        this.ohClaimIds.push(selectAllownace.ohClaimId);
      })
      // this.selectedAllowancelist = [];
      this.rejectEvent.emit({
        comments: event,
        ohClaimIds: this.ohClaimIds
      });
      // this.rejectEvent.emit(event);
      this.hideModal();
    }
    ...
    removeSelection(allowance) {
      this.selectedAllowancelist.forEach(element => {
        if (element.ohClaimId === allowance.ohClaimId) {
