import { Component, OnInit ,Input,Output,EventEmitter,Inject, TemplateRef, SimpleChanges,OnChanges} from '@angular/core';
...
//import {  EligibleRoleConstants, ContractAuthenticationService, ContributorRouteConstants } from '../../../shared';
import { BehaviorSubject } from 'rxjs';
import {AlertService} from '@gosi-ui/core';
import { BsModalRef, BsModalService } from 'ngx-bootstrap/modal';
import { ActivatedRoute, Router } from '@angular/router';
import { tap } from 'rxjs/operators';
import { EligibleRoleConstants } from '../../../shared';


const MaxEntriedReached = 'CONTRIBUTOR.ERROR.MAX-ENTRIES-OTP';

@Component({
  selector: 'mb-contact-otp-dc',
  templateUrl: './contact-otp-dc.component.html',
  styleUrls: ['./contact-otp-dc.component.scss']
})
export class ContactOtpDcComponent implements OnInit ,OnChanges{
  /** Local variables. */
  trim: string;
  message: string;
  _isValid = true;
  ...
  noOfResend =  EligibleRoleConstants.NO_OF_OTP_RETRIES; // Maximum no of resend possible
  ...
  /** Creates  an instance of ContractAuthDcComponent. */
    ...
    constructor(private fb: FormBuilder,readonly alertService: AlertService,private route: ActivatedRoute,readonly router: Router, readonly modalService: BsModalService, @Inject(LanguageToken) readonly language: BehaviorSubject<string>) {}
 ...
 /* initialiseFromRoute() {
  ...
  showOtpScreen() {
    this.clearAlert();
    this.isResend = false;
    if (this.noOfIncorrectOtp ===  EligibleRoleConstants.NO_OF_OTP_RETRIES) {
