...
import { EngagementPeriod, ModifyCoverage, ModifyEngagementPeriod, SystemParameter } from '@gosi-ui/features/contributor/lib/shared/models';
...
export class PeriodDetailsDcComponent extends WageDetailFormBase implements OnInit, OnChanges {
  ...
  showTemplate() {
    ...
    if (this.selectedPeriod.endDate && this.selectedPeriod.endDate.gregorian) {
      ...
      if (
        ...
        this.maxHijiriDate = this.ppaIndicator ? this.systemParameter.REG_CONT_MAX_END_DATE_H_PPA : '13/04/1439';
      }
    } else {
      this.isPeriodActive = true;
      this.maxGregorianDate = new Date();
      this.maxHijiriDate = this.ppaIndicator ? this.systemParameter.REG_CONT_MAX_END_DATE_H_PPA : this.maxHijiriDate; //'13/04/1439';
    }
  ...
  confirm() {
    if(!(this.checkCurrentMonth() && this.isSaudiPerson)){
  ...
  checkCurrentMonth(){
    ...
    if((currentDate.getMonth() == new Date(this.startMonthLabel.gregorian).getMonth())
    &&(currentDate.getFullYear() == new Date(this.startMonthLabel.gregorian).getFullYear()))
    {
      return true;
    }
    return false;
