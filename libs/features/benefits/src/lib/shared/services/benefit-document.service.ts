...
export class BenefitDocumentService {
  ...
  /** for calling the required docs for Modify Bank validator screen */
  getModifyBankDocuments(
    ...
    return this.modifyBenefitService.getReqDocsForModifyBank(sin, benefitRequestId, referenceNo).pipe(
      switchMap(res => {
        return forkJoin(
          res.map(doc => {
            return this.documentService.refreshDocument(
  ...
  getActiveHeirDocs(identifier: number, transactionId: number) {
    this.isIndividualApp = this.appToken === ApplicationTypeEnum.INDIVIDUAL_APP ? true : false;
    const encryptedTransactionId = this.cryptoService.encrypt(transactionId);
    ...
    return this.http.get<DocumentItem[]>(url).pipe(
      switchMap(res => {
        return forkJoin(
          res.map(doc => {
            return this.documentService.refreshDocument(doc, transactionId);
  ...
  getDocswithTxnId(transactionId: number) {
    this.isIndividualApp = this.appToken === ApplicationTypeEnum.INDIVIDUAL_APP ? true : false;
    const encryptedTransactionId = this.cryptoService.encrypt(transactionId);
    let url = `/api/v1/document/scanned-documents?`;
    if (transactionId) {
