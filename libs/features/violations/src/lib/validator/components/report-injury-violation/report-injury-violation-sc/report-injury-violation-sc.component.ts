...
@Component({
  selector: 'vol-report-injury-violation-sc',
  templateUrl: './report-injury-violation-sc.component.html',
  styleUrls: ['./report-injury-violation-sc.component.scss']
})
export class ReportInjuryViolationScComponent extends ValidatorBaseScComponent implements OnInit, OnDestroy {
  /**Local variables */
  ...
  verifyCompensated() {
    this.transactionDetails?.contributors.forEach((contributor, index) => {
      if (contributor) {
        const compensatedValue = (this.addEngagementForm.get('contributordetails') as FormArray)?.controls[index]
          .get('compensated')
          ?.get('english')?.value;
